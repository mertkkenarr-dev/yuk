...
import { suggestNextWeight, todayISO } from "../lib/calc";
...
export function SessionForm({ exercises, routines, sessions, onSave }: Props) {
  ...
  function addSet(exerciseId: string) {
    setDraft((d) =>
      d.map((x) => {
        ...
        return { ...x, sets: [...x.sets, last ? { ...last } : { weight: 0, reps: 0 }] };
      }),
    );
  }
  ...
  function updateSet(exerciseId: string, index: number, field: keyof SetEntry, value: number) {
    setDraft((d) =>
      d.map((x) =>
  ...
  function handleSave() {
    ...
    onSave({ date, routineDayName, exercises: exercisesOut });
    reset();
  }
                ...
                {d.sets.map((set, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="ml-auto text-ink-soft hover:text-power px-1"
                      aria-label="Seti sil"
                    >
                      ×
                    </button>
